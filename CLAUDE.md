...
- **분탄 월별 감가(동창 미지급)** → `monthly_depreciations` 테이블(품목×납품월), 입력 검증 `lib/depreciation.ts`, 계산서 반영 `coal.ts` genBuntan(동창 매입만 차감, 렘코 매출·커미션은 총액 기준), UI `invoices/DepreciationPanel.tsx`(입력·미정산 누계·정산완료)
