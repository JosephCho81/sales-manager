...
- **월별 감가는 품목 무관 공통 구조** → `monthly_depreciations` 테이블(품목×납품월×통보건), 검증·규칙 `lib/depreciation.ts`, UI는 `invoices/DepreciationPanel.tsx` 하나(품목 선택 + 통보일/귀속월/금액)
  **반영 위치는 담당자가 고르지 않는다 — 품목이 정한다.** `DEP_POLICIES`(lib/depreciation.ts)가 품목별로 매출 감액 여부·매입 차감처·회수월 선택 가능 여부를 갖고 있고, `depDeductMonths()`가 그걸로 `sales_deduct_ym`·`cost_deduct_ym` 두 컬럼을 만든다. 서버 액션이 클라이언트가 보낸 값이 아니라 이 결과를 저장한다
  ...
  - **소괴탄** `sales=year_month, cost=null`: 동국제강이 감가를 뺀 금액으로 역발행해 매출·커미션이 그달에 −. **렘코 매입은 100% 전액 지급하고 어느 계산서에서도 차감하지 않는다**
    - 이유는 렘코의 상장 준비다. 최종 납품처 감가를 렘코가 부담한 형태로 계산서에 찍히면 총액매출이 순액매출로 잡힌다. 그래서 `costParty: null`이고, 클라이언트가 회수월을 보내도 `depDeductMonths()`가 무시한다 — **렘코가 상대인 계산서에 감가를 붙이는 경로를 다시 만들지 말 것**
  ...
  - **패널 전체가 기본 접힘이다** — 이 페이지의 주인공은 계산서 목록이지 감가가 아니다. 펼치면 잔액 카드와 입력 폼이 먼저 나오고, 등록 내역은 그 안에서 한 번 더 접힌다(품목 단위, 접힌 줄은 품목·건수·합계). 감가가 쌓이면 행만 늘어나 계산서 표를 밀어낸다(사용자 피드백 2026-09-11)
  - **상단 미정산 감가는 회색 한 줄**이다(`InvoicesClient.tsx`). 빨간 경고 박스로 되돌리지 말 것 — 계산서 표보다 먼저 눈에 들어와 페이지 주목도가 뒤집힌다. 내역과 구성은 패널 안에 있으므로 여기엔 금액만 둔다
...
- **AL30 감가(현대 감액 발행 → 화림 회수)** → 같은 `monthly_depreciations` 테이블. `sales_deduct_ym`=매출 감액 납품월, `cost_deduct_ym`=매입 회수 납품월. `al30.ts`가 매출·매입 각각 차감하고 커미션(마진 1/3)도 같이 움직인다. 실입금액 입력은 `invoices/PaymentDialog.tsx` → `recordPaymentShortfall`. 상세 `docs/al30-depreciation-2026-05.md`
