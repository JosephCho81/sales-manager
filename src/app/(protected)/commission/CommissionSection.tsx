...
export default function CommissionSection({
  ...
  const preview = useMemo(
    () => commissionPreview(form.qty_ton, form.price_per_ton),
    [form.qty_ton, form.price_per_ton],
  )

  async function handleSave() {
    const v = validateCommissionInput(form.ym, form.qty_ton, form.price_per_ton)
    ...
    try {
      const result = await insertCommission({
        ...v.payload,
        company,
        memo: form.memo || `${company} ${form.ym} 커미션 — ${fmtNum(v.payload.quantity_kg / 1000, 3)}톤 × ${fmtNum(v.payload.price_per_ton)}원/톤`,
      })
