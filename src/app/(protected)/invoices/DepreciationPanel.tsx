    ...
    <div className="card mb-6 p-4">
      <h3 className="text-sm font-bold text-gray-900">
        ...
        <span className="ml-1 font-normal text-xs text-gray-400">
          — 품목을 고르면 어느 계산서에 반영할지는 자동으로 정해집니다
        </span>
      </h3>
      ...
      {(carries.length > 0 || pendings.length > 0) && (
        <div className="mt-3 flex flex-wrap gap-3">
          {carries.map(c => (
            <div key={c.party} className="rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs">
              <p className="font-bold text-amber-800">{c.party} 연말 정리</p>
              <p className="mt-1 text-amber-700 tabular-nums">
                받을 돈 {fmtKrw(c.receive)} · 돌려줄 돈 {fmtKrw(c.pay)}
              </p>
              <p className="mt-0.5 font-bold text-amber-900 tabular-nums">
                순액 {fmtKrw(Math.abs(c.net))} {c.net >= 0 ? '받을 돈' : '돌려줄 돈'}
              </p>
          ...
          {pendings.map(p => (
            <div key={p.party} className="rounded border border-blue-200 bg-blue-50 px-3 py-2 text-xs">
              <p className="font-bold text-blue-800">{p.party} 매입 회수 미완료</p>
              <p className="mt-1 font-bold text-blue-900 tabular-nums">{fmtKrw(p.amount)}</p>
              <p className="mt-1 text-blue-600">회수 계산서가 나가면 정산완료로 닫으세요.</p>
            </div>
          ))}
        </div>
      )}
