...
export async function upsertDelivery(payload: Record<string, unknown>, edit?: EditTarget) {
  ...
  if (edit) {
    ...
    await logAudit(auth.user, { table: 'deliveries', rowId: edit.id, action: 'update', after: payload })
    return { data: data[0] }
  } else {
    const { data, error } = await supabase
      .from('deliveries')
